...
class SmsServiceController extends Controller
{
    ...
    public function sendSms(Request $request)
    {
        ...
        if ($sending_type == 1) { //individual sms
                ...
                foreach ($individual_contacts as $individual_contact) {
                    if ($smsGateway && $smsGateway->provider_name == 'Reve') {
                        ...
                        $response = Http::get($smsGateway->api_endpoint, [
                            ...
                            'secretkey' => $smsGateway->secret_key,
                            "callerID" => $smsGateway->sender_id,
                            "toUser" => $this->formatBangladeshiPhoneNumber($request->individual_contact[0]),
                            "messageContent" => $template_description
                        ]);
                    } else {
                        Toastr::error('No SMS Api is Active', 'Failed');
            ...
            // return $data;
            $template_description = "Lorem ipsum dolor sit amet";
        ...
        try {
            ...
            if ($smsGateway && $smsGateway->provider_name == 'Reve') {
                $response = Http::get($smsGateway->api_endpoint, [
                    ...
                    'secretkey' => $smsGateway->secret_key,
                    "callerID" => $smsGateway->sender_id,
                    "toUser" => $request->sms_receivers,
                    "messageContent" => $request->template_description
                ]);
            } elseif ($smsGateway && $smsGateway->provider_name == 'ElitBuzz') {
                $response = Http::get($smsGateway->api_endpoint, [
                    'api_key' => $smsGateway->api_key,
                    "type" => "text",
                    "contacts" => $request->sms_receivers, //“88017xxxxxxxx,88018xxxxxxxx”
                    "senderid" => $smsGateway->sender_id,
                    "msg" => $request->template_description
                ]);
            } elseif ($smsGateway && $smsGateway->provider_name == 'KhudeBarta') {
                $response = Http::get($smsGateway->api_endpoint, [
                    'apikey' => $smsGateway->api_key,
                    'secretkey' => $smsGateway->secret_key,
                    "callerID" => $smsGateway->sender_id,
                    "toUser" => $this->formatBangladeshiPhoneNumber($request->sms_receivers),
                    "messageContent" => $request->template_description
                ]);
            } else {
                Toastr::error('No SMS Api is Active', 'Failed');
            ...
            Toastr::success('SMS send to Customer', 'Successfully');
            return back();
        } catch (\Throwable $th) {
            // return $th;
            return back();
