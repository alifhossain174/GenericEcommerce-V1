<tr>
    ...
    <td class="text-center" style="vertical-align: middle;">
        <select name="product_variant_id[]" onchange="productVariantInfo({{ $rowNo }})"
            id="product_variant_id_{{ $rowNo }}" class="form-control">
        </select>
    ...
    <td class="text-center" style="vertical-align: middle;">
        <input class="form-control d-inline-block w-50" type="number" onwheel="this.blur()"
            onkeyup="changeTotalPrice({{ $rowNo }})" min="1" name="qty[]" id="qty_{{ $rowNo }}"
            required> <span id="unit_text_{{ $rowNo }}"></span>
        <input type="hidden" name="unit_id[]" id="unit_{{ $rowNo }}">
    </td>
    <td class="text-center" style="vertical-align: middle;">
        {{ $currencySymbol }} <input class="form-control d-inline-block w-75" type="text" name="unit_price[]"
            id="unit_price_{{ $rowNo }}" readonly required>
    </td>
    <td class="text-right" style="vertical-align: middle;">
        {{ $currencySymbol }} <input class="form-control d-inline-block w-75 orderT" type="text"
            name="total_price[]" id="total_price_{{ $rowNo }}" readonly required>
    </td>
    <td class="text-center" style="vertical-align: middle;">
