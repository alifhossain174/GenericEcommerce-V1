...
$categoryModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where('route', 'like', '%add/new/category%')
    ->orWhere('route', 'like', '%view/new/category%')
    ->get();

$subCategoryModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where(function ($query) {
        $query->where('route', 'like', '%add/new/subcategory%')->orWhere('route', 'like', '%view/all/subcategory%');
    })
    ->get();

$childCategoryModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where('route', 'like', '%add/new/childcategory%')
...
$promoCodeModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where('route', 'like', '%add/new/code%')
    ->orWhere('route', 'like', '%view/all/promo/codes%')
    ->get();
$sliderBannerModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
...
$testimonialModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where('route', 'like', '%add/testimonial%')
    ->orWhere('route', 'like', '%view/testimonials%')
    ->get();
$pushNotificationModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where('route', 'like', '%send/notification/page%')
    ->orWhere('route', 'like', '%view/all/notifications%')
    ->get();
$smsServiceModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where('route', 'like', '%view/sms/templates%')
    ->orWhere('route', 'like', '%send/sms/page%')
    ->orWhere('route', 'like', '%view/sms/history%')
    ->get();
...
$blogModule = App\Models\UserRolePermission::where('user_id', Auth::user()->id)
    ->where('route', 'like', '%blog/categories%')
    ->orWhere('route', 'like', '%add/new/blog%')
...
<ul class="metismenu list-unstyled" id="side-menu">
    ...
    @if (checkAuth('general/info') ||
            checkAuth('website/theme/page') ||
            checkAuth('social/media/page') ||
            checkAuth('seo/homepage') ||
            checkAuth('custom/css/js') ||
            checkAuth('social/chat/script'))
    ...
    @if (env('MultiVendor') == true)
        ...
        @if (checkAuth('create/new/vendor') ||
                checkAuth('view/vendor/requests') ||
                checkAuth('view/all/vendors') ||
                checkAuth('view/inactive/vendors'))
            <li>
                <a href="javascript: void(0);" class="has-arrow"><i
                        class="feather-user-check"></i><span>Vendors</span></a>
        ...
        @if (checkAuth('create/new/store') || checkAuth('view/all/stores'))
            <li>
                ...
                <ul class="sub-menu" aria-expanded="false">
                    @if (checkAuth('create/new/store'))
                        <li><a href="{{ url('/create/new/store') }}">Create New Store</a></li>
                    @endif
                    @if (checkAuth('view/all/stores'))
                        <li><a href="{{ url('/view/all/stores') }}">View All Stores</a></li>
                    @endif
                </ul>
            </li>
        @endif
        @if (checkAuth('view/all/withdraws') ||
                checkAuth('create/new/withdraw') ||
                checkAuth('view/withdraw/requests') ||
                checkAuth('view/completed/requests') ||
                checkAuth('view/cancelled/requests'))
            <li>
                <a href="javascript: void(0);" class="has-arrow"><i
                ...
                <ul class="sub-menu" aria-expanded="false">
                    ...
                    @endif
                    @if (checkAuth('create/new/withdraw'))
                        <li><a href="{{ url('create/new/withdraw') }}">Create Withdrawal</a></li>
                    @endif
                    @if (checkAuth('view/withdraw/requests'))
                    ...
                    @endif
                    @if (checkAuth('view/completed/requests'))
                        <li><a href="{{ url('view/completed/withdraws') }}">Completed Withdrawals</a></li>
                    @endif
                    @if (checkAuth('view/cancelled/requests'))
    ...
    @if (count($configModule) > 0 ||
            count($categoryModule) > 0 ||
            count($subCategoryModule) > 0 ||
            count($childCategoryModule) > 0 ||
            count($productModule) > 0 ||
            count($orderModule) > 0 ||
            count($promoCodeModule) > 0 ||
            count($pushNotificationModule) > 0 ||
            count($smsServiceModule) > 0 ||
            count($systemModule) > 0 ||
            checkAuth('view/all/customers') ||
            checkAuth('view/customers/wishlist') ||
            checkAuth('view/delivery/charges') ||
            count($reportModule) > 0 ||
            count($backupModule) > 0)
        <hr style="border-color: #c8c8c836; margin-top: 12px; margin-bottom: 12px;">
    ...
    @if ($productModule && count($productModule) > 0)
        <li>
            ...
            <ul class="sub-menu" aria-expanded="false">
                ...
                @endif
                @if (checkAuth('products/from/excel'))
                    <li><a href="{{ url('/products/from/excel') }}">Bulk Upload <sup
                                class="text-warning">New</sup></a></li>
                @endif
                @if (checkAuth('bulk/product/update'))
            ...
            <ul class="sub-menu" aria-expanded="false">
                @if (env('POS') == true && env('POS_KEY') == 'GenericCommerceV1-SBW7583837NUDD82')
                    @if (checkAuth('create/new/order'))
                        <li><a style="color: white !important;" href="{{ url('/create/new/order') }}">Create
                                Order</a></li>
                    @endif
                @endif
                ...
                @if (checkAuth('view/orders'))
                    <li>
                        <a style="color: white !important;" href="{{ url('/view/orders') }}">
                            All Orders (@php echo DB::table('orders')->count(); @endphp)
                        </a>
                    </li>
                    ...
                    <li @if (request()->query('status') == 'ready-to-ship') class="mm-active" @endif>
                        <a @if (request()->query('status') == 'ready-to-ship') class="active" @endif style="color: wheat !important;"
                            ...
                            Ready to Ship ({{ DB::table('orders')->where('order_status', 1)->count() }})
                        </a>
                    </li>
                    <li @if (request()->query('status') == 'intransit') class="mm-active" @endif>
                        <a @if (request()->query('status') == 'intransit') class="active" @endif
                            style="color: lightgreen !important;" href="{{ url('/view/orders') }}?status=intransit">
    ...
    @if (checkAuth('view/delivery/charges'))
        <li><a href="{{ url('/view/delivery/charges') }}"><i class="feather-truck"></i><span>Delivery
                    Charges</span></a></li>
    @endif
    @if (checkAuth('view/upazila/thana'))
        <li><a href="{{ url('/view/upazila/thana') }}"><i class="dripicons-location"></i><span>Upazila &
                    Thana</span></a></li>
    @endif
        ...
        <li>
            ...
            <ul class="sub-menu" aria-expanded="false">
                @if (checkAuth('pending/support/tickets'))
                    <li><a style="color: skyblue !important;" href="{{ url('/pending/support/tickets') }}">Pending
                            Supports
                            (@php
                ...
                @if (checkAuth('solved/support/tickets'))
                    <li><a style="color: #0c0 !important;" href="{{ url('/solved/support/tickets') }}">Solved
                            Supports
                            (@php
                ...
                @if (checkAuth('on/hold/support/tickets'))
                    <li><a style="color: goldenrod !important;" href="{{ url('/on/hold/support/tickets') }}">On Hold
                            Supports
                            (@php
                ...
                @if (checkAuth('rejected/support/tickets'))
                    <li><a style="color: red !important;" href="{{ url('/rejected/support/tickets') }}">Rejected
                            Supports
                            (@php
    ...
    @if (count($sliderBannerModule) > 0 ||
            count($termsPolicyModule) > 0 ||
            count($testimonialModule) > 0 ||
            count($customPageModule) > 0 ||
            checkAuth('about/us/page') ||
            checkAuth('view/all/faqs') ||
            count($blogModule) > 0)
        <hr style="border-color: #c8c8c836; margin-top: 12px; margin-bottom: 12px;">
    ...
    @if (checkAuth('view/permission/routes') ||
            checkAuth('view/user/roles') ||
            checkAuth('view/user/role/permission') ||
            checkAuth('view/system/users'))
        <hr style="border-color: #c8c8c836; margin-top: 12px; margin-bottom: 5px;">
    ...
    @if (checkAuth('view/system/users'))
        <li><a href="{{ url('/view/system/users') }}"><i class="fas fa-user-shield"></i><span>System
                    Users</span></a></li>
    @endif
    @if (checkAuth('view/permission/routes'))
    ...
    @if (checkAuth('view/user/roles'))
        <li><a href="{{ url('/view/user/roles') }}"><i class="feather-user-plus"></i><span>User Roles</span></a>
        </li>
    @endif
